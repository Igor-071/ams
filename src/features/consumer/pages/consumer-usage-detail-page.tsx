...
import { getUsageRecordsByDateForConsumer, getServiceById } from '@/mocks/handlers.ts'
...
const COLUMNS: { label: string; sortKey: SortKey }[] = [
  { label: 'API Key', sortKey: 'apiKeyId' },
...
export function ConsumerUsageDetailPage() {
  ...
  const result = useMemo(
    () =>
      getUsageRecordsByDateForConsumer(date ?? '', consumerId, {
        page,
        pageSize: 20,
        sortBy,
        sortOrder: sortDir,
      }),
    [date, consumerId, page, sortBy, sortDir],
  )
  ...
  const allRecordsForExport = useMemo(
    ...
    [date, consumerId, sortBy, sortDir],
  )

  const handleExport = () => {
    const columns: CsvColumn<(typeof allRecordsForExport.data)[0]>[] = [
      { header: 'API Key', accessor: (r) => r.apiKeyId },
      ...
      <div className="space-y-6">
        <PageHeader
          ...
          breadcrumbs={[
            ...
            { label: 'Usage', href: ROUTES.CONSUMER_USAGE },
            { label: date ?? 'Unknown' },
          ]}
        />
        <Card>
